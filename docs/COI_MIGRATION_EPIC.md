...
- [ ] **Deferred:** CertificateNumberService — keep old system behavior for now; do not migrate in MVP
...
- Do not change certificate numbering behavior in MVP (deferred)
...
**Certificate Number:**
Deferred for MVP — continue using the existing approach in the old system. Future work may implement an atomic counter via `findOneAndUpdate({ $inc })`.
...
- [ ] **Data Layer Tests** (`src/data/`) - Test all ported functions from Story 1:
  ...
  - Certificate numbering behavior remains in old system; tests deferred
